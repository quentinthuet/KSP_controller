...
##                      @file: krpc_MEGA_communication                       ##
##    @description: Processing of communications from/to krpc mod to/from    ##
##                  Arduino MEGA                                             ##
###############################################################################
...
conn = krpc.connect(name='Soyouz', address='127.0.0.1', rpc_port=50000,       \
    stream_port=50001)
    ...
    if data:
        ...
        if data[21]:
            if x_translation_data:
                x_translation_valeur = round(float(x_translation_data)        \
                    / 512 - 1, 2)
                vessel.control.forward = x_translation_valeur
            if y_translation_data:
                y_translation_valeur = round(float(y_translation_data)        \
                    / 512 - 1, 2)
                vessel.control.up = y_translation_valeur
            if z_translation_data:
                z_translation_valeur = round(float(z_translation_data)        \
                    / 512 - 1, 2)
        ...
        if data[23]:    
            if zoom_data:
                zoom_valeur = round(float(zoom_data) / 1024                   \
     ...
     ### krpc reading and Arduino data writing (all digital) ###
    
    tosend = str(int(vessel.control.rcs)) + '.'                               \
        + str(int(vessel.control.sas)) + '.'                                  \
        + str(int(vessel.control.lights)) + '.'                               \
        + str(int(vessel.control.gear)) + '.'                                 \
        + str(int(vessel.control.brakes)) + '.'                               \
        + str(int(vessel.control.solar_panels)) + '.'                         \
        + str(int(vessel.control.antennas)) + '.'                             \
